...
void SIMGPRS::init()
{
	...
	if (!waitForResponse_F(FS("AT+CREG?"), 100, DEFAULT_TIMEOUT, FS("0,"), DEFAULT_RESPONSE_ERROR, nullptr))
	{
	...
	waitForResponse_F(FS("AT+CVHU=0"), DEFAULT_TRIES, DEFAULT_TIMEOUT, DEFAULT_RESPONSE_OK, DEFAULT_RESPONSE_ERROR, nullptr);
	setCallAutoAnswer(0);
...
boolean SIMGPRS::requestHTTP(RequestMethod method, String * host, String * parameters, uint16_t* resultCode, String* serviceResponse)
{
	...
	int dataLength = result.substring(result.lastIndexOf(F(",")) + 1, result.length()).toInt();

	result = "";
	if (serviceResponse != nullptr) {
		if (waitForResponse_str(String("AT+HTTPREAD=0,") + String(dataLength), 1, DEFAULT_TIMEOUT, FS("+HTTPREAD:"), DEFAULT_RESPONSE_ERROR, &result)) {
...
void SIMGPRS::_checkCallingStatus(String * str)
{
	...
	if (str->indexOf(FS("+CLIP")) >= 0 && str->indexOf(FS("AT+CLIP")) == -1)
	{
		PowerStatus lastPowerStatus = powerStatus();
		if (lastPowerStatus != PowerStatus::FULL_FUNC) {
			wakeUp();
